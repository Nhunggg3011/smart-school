...
import { useClassEvents } from 'store/students'
import moment from 'moment'
import { getCurrentWeekDay } from 'utils/utils'
...
const Profile = () => {
  ...
  const [events, loading, error, actions] = useClassEvents()

  const sortedEvents = useMemo(() => (events || []).sort((a, b) => (a.startTime || '').localeCompare((b.startTime || ''))), [events])

  useEffect(() => {
    if (user?.uid) {
      actions.fetchEvents(user.class, getCurrentWeekDay())
    }
  }, [user?.uid])
          ...
          <div className={classes.headerTitleContainer}>
            <div>
              <Typography variant="h2">
                {`Lịch học Online - Lớp ${user?.class}`}
              </Typography>
              <Typography variant="h8">{`Xin chào, bạn ${user?.lastName}`}</Typography>
            </div>
          </div>
        </Container>
      </LayoutHeader>
        ...
        <Grid container spacing={4} className={classes.paper}>
          <Grid item md={8} xs={12}>
            <Grid container spacing={2}>
              {currentEvents ? (
                <GridItem title="Giờ học đang diễn ra" xs={12}>
                  {renderEvent(currentEvents)}
                </GridItem>
              ) : ''}
              {nextEvent ? (
                <GridItem title="Giờ học tiếp theo" xs={12}>
                  {renderEvent(nextEvent)}
                </GridItem>
              ) : ''}
              {!currentEvents && !nextEvent ? (
                <GridItem title="Giờ học tiếp theo" xs={12}>
                  Hiện bạn không có tiết học nào trong hôm nay
                </GridItem>
              ) : ''}
            </Grid>
          </Grid>
          <Grid item md={4} xs={12}>
            <Grid container spacing={2}>
              <GridItem title="Lịch học trong ngày" xs={12}>
                <Grid container spacing={1}>
                  <Grid item xs={12}>
                    <table className={classes.listPlan}>
                      ...
                      {(events || []).map((e) => (
                        <tr key={e._id} onClick={() => history.push(`/phong-hoc?eventId=${e._id}&date=${moment().format('YYYY-MM-DD')}`)}>
                          ...
                          <td align="center">{SUBJECT_MAP[e.subject]?.label}</td>
